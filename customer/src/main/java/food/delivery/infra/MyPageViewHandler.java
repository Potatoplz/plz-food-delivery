...
public class MyPageViewHandler {
    ...
    public void whenCookingStarted_then_UPDATE_4(@Payload CookingStarted cookingStarted) {
        try {
            if (!cookingStarted.validate()) return;
                // view 객체 조회
            Optional<MyPage> myPageOptional = myPageRepository.findById(cookingStarted.getOrderId());
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void whenCookingFinished_then_UPDATE_5(@Payload CookingFinished cookingFinished) {
        try {
            if (!cookingFinished.validate()) return;
                // view 객체 조회
            Optional<MyPage> myPageOptional = myPageRepository.findById(cookingFinished.getOrderId());
    ...
    public void whenDeliveryCompleted_then_UPDATE_7(@Payload DeliveryCompleted deliveryCompleted) {
        try {
            if (!deliveryCompleted.validate()) return;
                // view 객체 조회
            Optional<MyPage> myPageOptional = myPageRepository.findById(deliveryCompleted.getOrderId());
